...
abstract contract BaseScript is Script, VaultUtils {
    ...
    function _configureDefaultRoles(KernelStrategy vault_) internal {
        ...
        vault_.grantRole(vault_.PROCESSOR_ROLE(), actors.PROCESSOR());
    ...
    function _loadDeployment() internal {
        ...
        viewer = KernelVaultViewer(payable(address(vm.parseJsonAddress(jsonInput, ".viewer-proxy"))));
        viewerImplementation =
            KernelVaultViewer(payable(address(vm.parseJsonAddress(jsonInput, ".viewer-implementation"))));
