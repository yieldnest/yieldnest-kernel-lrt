...
contract YnBitFiBTCkTest is Test, MainnetKernelActors, EtchUtils, VaultUtils, VaultKernelUtils {
    ...
    function configureKernelStrategy(KernelStrategy vault_) public {
        ...
        vault_.addAsset(MC.BTCB, false);
        vault_.addAsset(MC.BFBTC, true);
        vault_.setAssetWithdrawable(MC.BFBTC, true);
    ...
    function test_Vault_ERC4626_view_functions() public view {
        ...
        {
            // Test convertToBase for BTCB
            uint256 baseAmount = _convertAssetToBase(MC.BTCB, amount);
            uint256 convertedAmount = _convertBaseToAsset(MC.BTCB, baseAmount);

            assertEq(convertedAmount, amount, "Converted amount should be equal to amount");

            // Test convertToBase for BFBTC
            uint256 baseAmount2 = _convertAssetToBase(MC.BFBTC, amount);
            uint256 convertedAmount2 = _convertBaseToAsset(MC.BFBTC, baseAmount2);

            assertEq(convertedAmount2, amount, "Converted amount should be equal to amount");
        }

        {
            ...
            assertEq(shares, amount, "Shares for BTCB should be as expected");
        ...
        {
            uint256 bfbtcAmount = IBFBTC(MC.BFBTC).previewDeposit(amount);

            uint256 bfbtcAmountConverted = _convertBaseToAsset(MC.BFBTC, amount);

            assertEq(bfbtcAmountConverted, bfbtcAmount, "BFBTC amount should be equal to base amount converted");

            uint256 bfbtcBaseAssets = _convertAssetToBase(MC.BFBTC, bfbtcAmount);

            assertApproxEqAbs(bfbtcBaseAssets, amount, 2, "BFBTC base assets should be equal to amount");

            (uint256 bfbtcShares,) = _convertToShares(MC.BFBTC, bfbtcAmount);

            (uint256 btcbShares, uint256 btcbBaseAssets) = _convertToShares(MC.BTCB, amount);

            assertEq(btcbBaseAssets, amount, "BTCB base assets should be equal to amount");
            assertApproxEqAbs(bfbtcBaseAssets, btcbBaseAssets, 2, "BFBTC base assets should be equal to BTCB amount");
            assertApproxEqAbs(bfbtcShares, btcbShares, 2, "BFBTC shares should be equal to BTCB shares");

            (uint256 convertedAssets,) = _convertToAssets(MC.BFBTC, bfbtcShares);

            assertApproxEqAbs(convertedAssets, bfbtcAmount, 2, "Converted assets should be equal to amount");
        }
        ...
        assertEq(assets.length, 2, "There should be 1 assets in the vault");
        assertEq(assets[0], MC.BTCB, "First asset should be BTCB");
        assertEq(assets[1], MC.BFBTC, "Second asset should be BFBTC");

        // Test the getAsset function for BTCB
        assertEq(vault.getAsset(MC.BTCB).decimals, 18, "Asset decimals should be 8");
        assertEq(vault.getAsset(MC.BTCB).active, false, "Asset should be active");
        assertEq(vault.getAsset(MC.BTCB).index, 0, "Asset index should be 0");
        ...
        assertEq(vault.getAsset(MC.BFBTC).index, 1, "Asset index should be 0");
    ...
    function test_Vault_ynBfBTCk_deposit_BTCB_ShouldRevert(uint256 amount) public {
        amount = bound(amount, 1, 100_000 ether);

        deal(MC.BTCB, bob, amount);
        ...
        vault.depositAsset(MC.BTCB, amount, bob);
    ...
    function test_Vault_ynBfBTCk_deposit_BFBTC(uint256 amount) public {
        ...
        uint256 bfbtcAmount = obtainBFBTC(bob, amount);

        uint256 shares = depositIntoVault(MC.BFBTC, bfbtcAmount);

        uint256 convertedAssets = vault.convertToAssets(shares);
        uint256 assetsInBase = _convertAssetToBase(MC.BFBTC, bfbtcAmount);
        assertApproxEqAbs(assetsInBase, convertedAssets, 2, "Assets in base should be equal to converted assets");

        // This tests that the amount of ynBfBTC received is close to the amount of BTCB deposited
        // 0.01% threshold here, but we are only testing the decimals
        assertApproxEqRel(shares, amount, 1e14, "Shares decimals should be equal to btcb amount decimals");

        // shares / 1e18 should be equal to BFBTC amount / 1e8
        // Test that shares by decimals is equal to BFBTC amount by decimals
        uint256 shareDecimals = vault.decimals();
        uint256 amountDecimals = IERC20(MC.BFBTC).decimals();

        // 1% threshold here, but we are only testing the decimals
        assertApproxEqRel(
            shares,
            bfbtcAmount * 10 ** (shareDecimals - amountDecimals),
            1e16,
            "Sharesdecimals should be equal to btcb amount decimals"
        );
    }
    ...
    function test_Vault_ynBfBTCk_deposit_BFBTC_WithKernelVault(uint256 amount) public {
        ...
        uint256 shares = _depositIntoVault_WithKernelVault(MC.BFBTC, bfbtcAmount);
        ...
        // This tests that the amount of ynBfBTC received is close to the amount of BTCB deposited
        // 0.01% threshold here, but we are only testing the decimals
        assertApproxEqRel(shares, amount, 1e14, "Shares decimals should be equal to btcbc amount decimals");

        // shares / 1e18 should be equal to BFBTCB amount / 1e8
        // Test that shares by decimals is equal to BFBTC amount by decimals
        uint256 shareDecimals = vault.decimals();
        uint256 amountDecimals = IERC20(MC.BFBTC).decimals();
        ...
        assertApproxEqRel(
            ...
            bfbtcAmount * 10 ** (shareDecimals - amountDecimals),
            1e16,
            "Sharesdecimals should be equal to bfbtc amount decimals"
        );
    }
    ...
    function test_Vault_ynBfBTCk_rateChanges(uint256 amount) public {
        amount = bound(amount, bfbtcMinDepositAmount, 100_000 ether);
        uint256 bfbtcAmount = obtainBFBTC(bob, amount);

        depositIntoVault(MC.BFBTC, bfbtcAmount);

        uint256 initialTotalAssets = vault.totalAssets();

        {
            // test initial total assets are correct
            assertEq(bfbtc.balanceOf(address(vault)), bfbtcAmount, "Vault should have the bfBTC");
            uint256 assetsInBase = _convertAssetToBase(MC.BFBTC, bfbtcAmount);
            assertApproxEqAbs(initialTotalAssets, assetsInBase, 2, "Total assets should be equal to assets in base");
        }

        uint256 currentRatio = bfbtc.currentRatio();
        uint256 currentEpoch = bfbtc.currentEpoch();
        uint256 rate = IProvider(vault.provider()).getRate(MC.BFBTC);

        {
            // test initial rate
            uint256 ratioFromEpoch = bfbtc.ratio(currentEpoch - 1);
            assertEq(ratioFromEpoch, currentRatio, "Ratio from epoch should be equal to current ratio");

            uint256 previewedWithdraw = bfbtc.previewWithdraw(1e8, false);
            assertEq(rate, previewedWithdraw, "Rate should be equal to previewed withdraw");
        }

        {
            // test rate changes according to the new ratio

            // change rate so bfBTC is more valuable now
            uint256 newRatio = currentRatio * 98 / 100;
            vm.startPrank(bfbtc.multisig());
            bfbtc.updateEpoch(newRatio);
            vm.stopPrank();

            uint256 newCurrentEpoch = bfbtc.currentEpoch();
            assertEq(newCurrentEpoch, currentEpoch + 1, "New epoch should be equal to current epoch + 1");

            uint256 newCurrentRatio = bfbtc.currentRatio();
            uint256 newRatioFromEpoch = bfbtc.ratio(currentEpoch);
            assertEq(newCurrentRatio, newRatioFromEpoch, "New current ratio should be equal to new ratio from epoch");
            assertEq(newRatio, newCurrentRatio, "New ratio should be equal to new current ratio");

            uint256 newRate = IProvider(vault.provider()).getRate(MC.BFBTC);
            uint256 previewedWithdraw = bfbtc.previewWithdraw(1e8, false);
            assertEq(newRate, previewedWithdraw, "New rate should be equal to previewed withdraw");

            assertGt(newRate, rate, "New rate should be greater than initial rate");
        }

        {
            // test final total assets are correct
            assertEq(bfbtc.balanceOf(address(vault)), bfbtcAmount, "Vault should have the same amount of bfBTC");
            uint256 assetsInBase = _convertAssetToBase(MC.BFBTC, bfbtcAmount);

            uint256 finalTotalAssets = vault.totalAssets();
            assertApproxEqAbs(finalTotalAssets, assetsInBase, 2, "Total assets should be equal to assets in base");

            assertGt(finalTotalAssets, initialTotalAssets, "Total assets should be greater than initial total assets");
        }

        {
            // change rate so bfBTC is less valuable now
            uint256 newRatio = currentRatio * 100 / 98;
            ...
            uint256 newRate = IProvider(vault.provider()).getRate(MC.BFBTC);
            assertLt(newRate, rate, "New rate should be less than initial rate");
        }

        {
            // test final total assets are correct
            uint256 finalTotalAssets = vault.totalAssets();
            assertLt(finalTotalAssets, initialTotalAssets, "Total assets should be less than initial total assets");
        }
    }
}
